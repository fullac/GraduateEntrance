...
35. 无法通过内存释放函数free(void*)直接将某块已经使用完的物理内存直接还给操作系统，因为可能要等到进程结束才能释放。
36. **C语言进程的逻辑地址分区：**
...
37. **可执行程序的三段：**
    ...
    - `.data`：已初始化以及常量；
    - `.text`：代码。
38. 空结构体的大小为0字节（用编译器试过），但是sbGPT说C语言空结构体会报错或是大小为1字节。
39. 什么是字符串比较大小：比较的是每位的ASCII码值大小。
40. 常量指针：`const int* p;`or`int const* p;`
41. 指针常量：`int *const p;`
42. C语言结构体中不允许有函数，但是C++允许
